...
public class ChessBoard
{
	...
	static
	{
		...
		map.put('８', "八");
		map.put('９', "九");
		sound = new SoundPlayer(PathUtils.getJarPath() + "sound/click.wav", PathUtils.getJarPath() + "sound/move.wav",
				PathUtils.getJarPath() + "sound/capture.wav", PathUtils.getJarPath() + "sound/check.wav", PathUtils.getJarPath() + "sound/win.wav");
	}
	...
	private void setNewBoard(String fenCode)
	{
		...
		for (int i = 0; i < 10; i++)
		{
			for (int j = 0; j < 9; j++)
			{
				...
				else if (i == 0 && (j == 1 || j == 7)) // 黑马
				{
					board[i][j] = 'n';
				}
				else if (i == 0 && (j == 2 || j == 6))// 黑象
				{
					board[i][j] = 'b';
				}
				else if (i == 0 && (j == 3 || j == 5)) // 黑士
				{
					board[i][j] = 'a';
				}
				else if (i == 0 && j == 4) // 黑将
				...
				else if (i == 2 && (j == 1 || j == 7)) // 黑炮
				{
					board[i][j] = 'c';
				}
				else if (i == 3 && (j == 0 || j == 2 || j == 4 || j == 6 || j == 8)) // 黑卒
				{
					board[i][j] = 'p';
				}
				else if (i == 9 && (j == 0 || j == 8))  // 红车
				{
					board[i][j] = 'R';
				}
				else if (i == 9 && (j == 1 || j == 7))  // 红马
				{
					board[i][j] = 'N';
				}
				else if (i == 9 && (j == 2 || j == 6)) // 红相
				{
					board[i][j] = 'B';
				}
				else if (i == 9 && (j == 3 || j == 5)) // 红仕
				{
					board[i][j] = 'A';
				}
				else if (i == 9 && j == 4) // 红帅
				...
				else if (i == 7 && (j == 1 || j == 7)) // 红炮
				{
					board[i][j] = 'C';
				}
				else if (i == 6 && (j == 0 || j == 2 || j == 4 || j == 6 || j == 8)) // 红兵
				{
					board[i][j] = 'P';
				}
				else
	...
	public String mouseClick(int x, int y, boolean canRedGo, boolean canBlackGo)
	{
		...
		if (remarkPoint != null)
		{
			boolean isRed = XiangqiUtils.isRed(board[remarkPoint.y][remarkPoint.x]);
			...
			else if (board[j][i] != ' ' && XiangqiUtils.isRed(board[j][i]) == isRed)
			{
				...
				remarkPoint = new Point(i, j);
				paint();
				return null;
			}
			else if (!XiangqiUtils.canGo(board, remarkPoint.y, remarkPoint.x, j, i))
			{
				return null;
			}
			else
			{
				return move(remarkPoint.x, remarkPoint.y, i, j);
			}
		}
		else
		{
	...
	private void paint()
	{
		this.boardRender.paint(boardSize, this.board, prevStep, remarkPoint, stepTip, tipFirst, tipSecond, isReverse);
	}
