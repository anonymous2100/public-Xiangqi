...
public abstract class BaseBoardRender implements BoardRender
{
	...
	public void paint(BoardSize boardSize, char[][] board, Step prevStep, Point remark, boolean stepTip, Step tipFirst,
			Step tipSecond, boolean isReverse)
	{
	 ...
	 * @return
	 */
	private double getCenterTextSize(BoardSize style)
	{
		switch (style)
		{
			case BIG_BOARD:
	...
	public void drawStepTips(int pos, int piece, int x1, int y1, int x2, int y2, boolean isReverse, boolean isFirst)
	{
		...
		gc.fillPolygon(new double[] { x1, x2 + offX, x2 + offX + h / 2, x2, x2 + offX + h / 2, x2 + offX, x1 },
				new double[] { y1 - offY, y1 - offY, y1 - offY - h, y1, y1 + offY + h, y1 + offY, y1 + offY }, 7);
		gc.restore();
	}
	 ...
	 * @return
	 */
	private double getStepRectWitdh(BoardSize style)
	{
		switch (style)
		{
			case BIG_BOARD:
	...
	@Override
	public void drawStepRemark(int pos, int piece, int x, int y, boolean isPrevStep, boolean isReverse, BoardSize style)
	{
		x = pos + piece * x;
		...
		gc.setStroke(color);
		gc.strokePolyline(new double[] { x - len / 2 + len / 6, x - len / 2, x - len / 2 }, new double[] { y - len / 2, y - len / 2, y - len / 2 + len / 6 },
				3);
		gc.strokePolyline(new double[] { x - len / 2 + len / 6, x - len / 2, x - len / 2 }, new double[] { y + len / 2, y + len / 2, y + len / 2 - len / 6 },
				3);
		gc.strokePolyline(new double[] { x + len / 2 - len / 6, x + len / 2, x + len / 2 }, new double[] { y - len / 2, y - len / 2, y - len / 2 + len / 6 },
	...
	@Override
	public void drawBoardLine(int pos, int padding, int piece, boolean isReverse, BoardSize style)
	{
		// 棋盘竖线横线
	...
	private void drawStarPos(int x, int y, int w, String style)
	{
		...
		if (style.contains("l"))
		{
			gc.strokePolyline(new double[] { x - offset - len, x - offset, x - offset }, new double[] { y - offset, y - offset, y - offset - len }, 3);
			gc.strokePolyline(new double[] { x - offset - len, x - offset, x - offset }, new double[] { y + offset, y + offset, y + offset + len }, 3);
		}
		if (style.contains("r"))
		{
	 ...
	 * @return
	 */
	private double getNumberSize(BoardSize style)
	{
		switch (style)
		{
			case BIG_BOARD:
	 ...
	 * @return
	 */
	private double getInnerRectWidth(BoardSize style)
	{
		switch (style)
		{
			case BIG_BOARD:
	 ...
	 * @return
	 */
	private double getOutRectWidth(BoardSize style)
	{
		switch (style)
		{
			case BIG_BOARD:
	 ...
	 * @return
	 */
	public int getPieceSize(BoardSize style)
	{
		switch (style)
		{
			case BIG_BOARD:
	 ...
	 * @return
	 */
	public int getPadding(BoardSize style)
	{
		switch (style)
		{
			case BIG_BOARD:
