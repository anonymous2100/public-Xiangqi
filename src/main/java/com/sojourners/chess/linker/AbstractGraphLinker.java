...
public abstract class AbstractGraphLinker implements GraphLinker, Runnable
{
	...
	class Action
	{
		...
		{
			return "Action{" + "flag=" + flag + ", x1=" + x1 + ", y1=" + y1 + ", x2=" + x2 + ", y2=" + y2 + '}';
		}
	}

	/**
	 * 对比棋盘，计算出当前操作 flag： 1对方已走棋，需要同步到引擎 2引擎已走棋，需要同步到目标平台 3识别到新棋局 4可能识别到新棋局
	 * 
	 * @param linkBoard
	 * @param engineBoard
	 * @param robotBlack
		...
		for (int i = 0; i < diffList.size(); i++)
		{
			for (int j = i + 1; j < diffList.size(); j++)
			{
				...
				if (linkBoard[p1.x][p1.y] == engineBoard[p2.x][p2.y] && linkBoard[p1.x][p1.y] != ' ')
				{
					...
					{
						if (analysisMode || robotBlack && XiangqiUtils.isRed(linkBoard[p1.x][p1.y])
								|| !robotBlack && !XiangqiUtils.isRed(linkBoard[p1.x][p1.y]))
						{
							flag = 1;
							from = p2;
							to = p1;
						...
						else if (robotBlack && !XiangqiUtils.isRed(linkBoard[p1.x][p1.y])
								|| !robotBlack && XiangqiUtils.isRed(linkBoard[p1.x][p1.y]))
						{
							flag = 2;
							from = p1;
					...
					if (linkBoard[p2.x][p2.y] == ' ' && engineBoard[p1.x][p1.y] != ' '
							&& XiangqiUtils.isRed(linkBoard[p1.x][p1.y]) != XiangqiUtils.isRed(engineBoard[p1.x][p1.y]))
					{
						flag = 1;
						from = p2;
						to = p1;
					...
					if (!analysisMode && engineBoard[p1.x][p1.y] == ' ' && linkBoard[p2.x][p2.y] != ' '
							&& XiangqiUtils.isRed(engineBoard[p2.x][p2.y]) != XiangqiUtils.isRed(linkBoard[p2.x][p2.y]))
					{
						flag = 2;
						from = p1;
						to = p2;
				...
				if (linkBoard[p2.x][p2.y] == engineBoard[p1.x][p1.y] && linkBoard[p2.x][p2.y] != ' ')
				{
					...
					{
						if (analysisMode || robotBlack && XiangqiUtils.isRed(linkBoard[p2.x][p2.y])
								|| !robotBlack && !XiangqiUtils.isRed(linkBoard[p2.x][p2.y]))
						{
							flag = 1;
							from = p1;
							to = p2;
						...
						else if (robotBlack && !XiangqiUtils.isRed(linkBoard[p2.x][p2.y])
								|| !robotBlack && XiangqiUtils.isRed(linkBoard[p2.x][p2.y]))
						{
							flag = 2;
							from = p2;
					...
					if (linkBoard[p1.x][p1.y] == ' ' && engineBoard[p2.x][p2.y] != ' '
							&& XiangqiUtils.isRed(linkBoard[p2.x][p2.y]) != XiangqiUtils.isRed(engineBoard[p2.x][p2.y]))
					{
						flag = 1;
						from = p1;
						to = p2;
					...
					if (!analysisMode && engineBoard[p2.x][p2.y] == ' ' && linkBoard[p1.x][p1.y] != ' '
							&& XiangqiUtils.isRed(engineBoard[p1.x][p1.y]) != XiangqiUtils.isRed(linkBoard[p1.x][p1.y]))
					{
						flag = 2;
						from = p2;
						to = p1;
				...
				if (f && (flag == 1 && XiangqiUtils.canGo(engineBoard, from.x, from.y, to.x, to.y)
	...
	private boolean validateChessBoard(char[][] board)
	{
		for (int i = 0; i < 10; i++)
		{
			for (int j = 0; j < 9; j++)
			{
				...
				if ((board[i][j] == 'a' || board[i][j] == 'A') && ((i > 2 && i < 7) || j < 3 || j > 5
						|| (i <= 2 && (i + j) % 2 == 0) || (i >= 7 && (i + j) % 2 == 1)))
				{
					return false;
				}
			}
