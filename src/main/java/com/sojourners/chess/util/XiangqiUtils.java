...
public class XiangqiUtils
{
	...
	public static boolean isJiang(char[][] board, boolean isRed)
	{
		...
		if (isRed
				&& (board[y - 2][x + 1] == 'n' && board[y - 1][x + 1] == ' '
						|| board[y - 2][x - 1] == 'n' && board[y - 1][x - 1] == ' ')
				|| !isRed && (y - 2 >= 0) && (board[y - 2][x + 1] == 'N' && board[y - 1][x + 1] == ' '
						|| board[y - 2][x - 1] == 'N' && board[y - 1][x - 1] == ' '))
		{
			return true;
		}
		if (isRed && (y + 2 < 10)
				&& (board[y + 2][x + 1] == 'n' && board[y + 1][x + 1] == ' '
						|| board[y + 2][x - 1] == 'n' && board[y + 1][x - 1] == ' ')
				|| !isRed && (board[y + 2][x + 1] == 'N' && board[y + 1][x + 1] == ' '
						|| board[y + 2][x - 1] == 'N' && board[y + 1][x - 1] == ' '))
		{
			return true;
		}
		if (isRed
				&& (board[y - 1][x - 2] == 'n' && board[y - 1][x - 1] == ' '
						|| y + 1 < 10 && board[y + 1][x - 2] == 'n' && board[y + 1][x - 1] == ' ')
				|| !isRed && (y - 1 >= 0 && board[y - 1][x - 2] == 'N' && board[y - 1][x - 1] == ' '
						|| board[y + 1][x - 2] == 'N' && board[y + 1][x - 1] == ' '))
		{
			return true;
		}
	...
	public static boolean isSha(char[][] board, boolean isRed)
	{
		for (int i = 0; i < board.length; i++)
		{
			...
			{
				if (!isRed && (board[i][j] >= 'a' && board[i][j] <= 'z')
						|| isRed && (board[i][j] >= 'A' && board[i][j] <= 'Z'))
				{
	...
	private static boolean jieJiang(char[][] board, boolean isRed, int x, int y)
	{
		switch (board[y][x])
		{
			...
			case 'K':
			{
				...
				if ((isRed && y - 1 >= 7 || !isRed && y - 1 >= 0)
						&& (board[y - 1][x] == ' ' || isRed(board[y - 1][x]) != isRed))
				{
			...
			case 'P':
			{
				...
				{
					if ((board[y][x - 1] == ' ' || isRed(board[y][x - 1]) != isRed)
							&& jieJiang(board, isRed, x, y, x - 1, y))
					{
						return true;
					}
				}
			...
			case 'A':
			{
				if (x - 1 >= 3 && y - 1 >= (isRed ? 7 : 0)
						&& (board[y - 1][x - 1] == ' ' || isRed(board[y - 1][x - 1]) != isRed))
				{
					if (jieJiang(board, isRed, x, y, x - 1, y - 1))
						return true;
				}
				if (x - 1 >= 3 && y + 1 <= (isRed ? 9 : 2)
						&& (board[y + 1][x - 1] == ' ' || isRed(board[y + 1][x - 1]) != isRed))
				{
					if (jieJiang(board, isRed, x, y, x - 1, y + 1))
						return true;
				}
				if (x + 1 <= 5 && y - 1 >= (isRed ? 7 : 0)
						&& (board[y - 1][x + 1] == ' ' || isRed(board[y - 1][x + 1]) != isRed))
				{
					if (jieJiang(board, isRed, x, y, x + 1, y - 1))
						return true;
				}
				if (x + 1 <= 5 && y + 1 <= (isRed ? 9 : 2)
						&& (board[y + 1][x + 1] == ' ' || isRed(board[y + 1][x + 1]) != isRed))
				{
					if (jieJiang(board, isRed, x, y, x + 1, y + 1))
						return true;
