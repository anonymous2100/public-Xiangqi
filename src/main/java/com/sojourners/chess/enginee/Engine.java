...
public class Engine
{
	...
	{
		FIXED_TIME, FIXED_STEPS, INFINITE;
	}
				...
				public void run()
				{
					try
					{
						...
						while ((line = finalBr.readLine()) != null)
						{
							...
							if (line.startsWith("option") && line.contains("name") && line.contains("type")
									&& line.contains("default") && !line.contains("Threads") && !line.contains("Hash"))
							{
...
//			}
	...
	private void bestMove(String msg)
	{
		...
		if (Properties.getInstance().getEngineDelayEnd() > 0
				&& Properties.getInstance().getEngineDelayEnd() >= Properties.getInstance().getEngineDelayStart())
		{
			int t = random.nextInt(Properties.getInstance().getEngineDelayEnd());
			sleep(t);
		}
	...
	public void analysis(String fenCode, List<String> moves, char[][] board, boolean redGo)
	{
		ExecutorsUtils.getInstance().exec(() -> {
			if (Properties.getInstance().getBookSwitch())
			{
				...
				List<BookData> results = OpenBookManager.getInstance().queryBook(board, redGo,
						moves.size() / 2 >= Properties.getInstance().getOffManualSteps());
				...
				if (results.size() > 0 && this.analysisModel != AnalysisModel.INFINITE)
				{
					...
					{
						int t = random.nextInt(Properties.getInstance().getBookDelayEnd());
						sleep(t);
					}
	...
	{
		String protocol = Engine.test("D:\\1_EclipseWorkspace\\engines\\ucci\\ELEEYE.EXE",
				new LinkedHashMap<>());
