...
import com.sojourners.chess.board.Step;
...
public class Controller implements EngineCallBack, LinkerCallBack
{
	...
	void boardStyleSelected(ActionEvent event)
	{
		...
		{
			prop.setBoardStyle(BoardStyle.DEFAULT);
		}
		else
		{
			prop.setBoardStyle(BoardStyle.CUSTOM);
		}
	...
	void boardSizeSelected(ActionEvent event)
	{
		...
		{
			prop.setBoardSize(BoardSize.BIG_BOARD);
		}
		else if (item.equals(menuOfMiddleBoard))
		{
			prop.setBoardSize(BoardSize.MIDDLE_BOARD);
		}
		else
		{
			prop.setBoardSize(BoardSize.SMALL_BOARD);
		}
	...
	public void canvasClick(MouseEvent event)
	{
		...
		{
			String move = board.mouseClick((int) event.getX(), (int) event.getY(), redGo && !robotRed.getValue(), !redGo && !robotBlack.getValue());
			if (move != null)
			{
				goCallBack(move);
			}
	...
	{
		DialogUtils.showInfoDialog("关于",
				"中国象棋界面" + System.lineSeparator() + "Built on : 20240309" + System.lineSeparator() + "Author : T" + System.lineSeparator() + "Version : 1.2");
	}
	...
	private void initChessBoard()
	{
		...
		// 棋盘大小
		if (prop.getBoardSize() == BoardSize.BIG_BOARD)
		{
			menuOfBigBoard.setSelected(true);
		}
		else if (prop.getBoardSize() == BoardSize.MIDDLE_BOARD)
		{
		...
		// 棋盘样式
		if (prop.getBoardStyle() == BoardStyle.DEFAULT)
		{
		...
		this.timeShowLabel.setText(prop.getAnalysisModel() == Engine.AnalysisModel.FIXED_TIME ? "固定时间" + prop.getAnalysisValue() / 1000d + "s"
	 ...
	 * 
	 * @param fenCode
	 *            传null 新建默认初始局面；传fenCode 则根据fen创建局面
	 */
	...
	private void initGraphLinker()
	{
		try
		{
			this.graphLinker = com.sun.jna.Platform.isWindows() ? new WindowsGraphLinker(this)
					: (com.sun.jna.Platform.isLinux() ? new LinuxGraphLinker(this) : new MacosGraphLinker(this));
		}
		catch (Exception e)
		{
			e.printStackTrace();
	 ...
	 * @param step
	 */
	private void trickAutoClick(Step step)
	{
		if (step != null)
		{
	...
	public void bestMove(String first, String second)
	{
		if (redGo && robotRed.getValue() || !redGo && robotBlack.getValue())
		{
			Step s = board.stepForBoard(first);
			Platform.runLater(() -> {
			...
			if (td.getValid())
			{
				Platform.runLater(() -> {
					...
					if (prop.isLinkShowInfo())
					{
						...
						timeShowLabel.setText(prop.getAnalysisModel() == Engine.AnalysisModel.FIXED_TIME ? "固定时间" + prop.getAnalysisValue() / 1000d + "s"
								: "固定深度" + prop.getAnalysisValue() + "层");
					}
	...
	private void switchPlayer(boolean f)
	{
		engineStop();
		graphLinker.pause();
		boolean tmpRed = robotRed.getValue(), tmpBlack = robotBlack.getValue(), tmpAnalysis = robotAnalysis.getValue(), tmpLink = linkMode.getValue(),
